...
export const useSupabase = () => {
  ...
  const projects = {
    // Create new project
    create: useCallback(
      async (projectData: DphProjectInsert): Promise<DphProject | null> => {
        setLoading(true);
        setError(null);
    ...
    getAll: useCallback(async (): Promise<DphProject[]> => {
      ...
      try {
        console.log(
          "useSupabase.getAll: Making Supabase query to dph_projects...",
        );
        const { data, error: dbError } = await supabase
          .from("dph_projects")
          .select("*")
        ...
        console.log(
          "useSupabase.getAll: Successfully fetched",
          data?.length || 0,
          "projects",
        );
        return data || [];
    ...
    update: useCallback(
      async (
        id: string,
        updates: DphProjectUpdate,
      ): Promise<DphProject | null> => {
        setLoading(true);
        setError(null);
  ...
  const projectFiles = {
    ...
    create: useCallback(
      async (
        fileData: DphProjectFileInsert,
      ): Promise<DphProjectFile | null> => {
        setLoading(true);
        setError(null);
    ...
    getByProjectId: useCallback(
      async (projectId: string): Promise<DphProjectFile[]> => {
        setLoading(true);
        setError(null);
    ...
    getStats: useCallback(async (): Promise<{
      ...
      try {
        ...
        const stats = {
          total: data?.length || 0,
          submitted:
            data?.filter((p: { status: string }) => p.status === "Submitted")
              .length || 0,
          inProgress:
            data?.filter((p: { status: string }) => p.status === "In Progress")
              .length || 0,
          completed:
            data?.filter((p: { status: string }) => p.status === "Completed")
              .length || 0,
          thisMonth:
            data?.filter((p: { status: string; created_at: string }) => {
